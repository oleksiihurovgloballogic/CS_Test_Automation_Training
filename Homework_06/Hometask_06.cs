namespace Homework_06
{
    internal class Hometask_06
    {
        ...
        /// 
        /// <remarks>
        /// Encapsulation is used in the Car class by hiding
        /// provided & stored data: VIN, WMI, VDS and VIS attributes from outside,
        /// and also hiding internal database of WMI-Codes,
        /// but making public the calculation methods, which provides valuable results:
        /// - GetWorldManufacturerRegion(),
        /// - GatWorldManufacturerCountry(),
        /// - GetWorldManufacturerModel(),
        /// - and GetModelYear().
        /// </remarks>
        /// 
        /// <remarks>
        /// Abstraction is used in the Car class by retrieving only such information,
        /// which is possible from the initially provided VIN-number.
        /// This is what the class is intended to be used for, no less no more.
        /// Any additional or distinct output information will require
        /// more input data to operate on, so is not implemented here.
        /// </remarks>
        public class Car
        {
            // --- Attributes --------------------------------------------------
            ...
            private static List<WMI_Codes> WMICodesList = File.ReadAllLines("WMI_Codes.csv")
                .Skip(1)
                .Select(values => WMI_Codes.FromCsv(values))
                .ToList();
            // Particular WMI code index for the Car instance
            private int WMICodeIndex { get; set; }
            ...
            public string GetWorldManufacturerCountry()
            {
                if (WMICodeIndex != -1)
                {
            ...
            public void PrintCarInfo()
            {
                ...
                Console.WriteLine($"Manufacturer Country:\t{GetWorldManufacturerCountry()}");
        ...
        public static void Main()
        {
            ...
            Console.WriteLine("\n------ Example of incorrect Car class methods usage --------------------\n");

            string myExceptionalVIN = "5YJYGDEF2LFR";
            try
            {
                var myExceptionalCar = new Car(myExceptionalVIN);
            }
            catch (ArgumentException ex)
            {
                Console.WriteLine($"Error message: {ex.Message}");
                throw new InvalidOperationException(
                    $"Please recheck the argument value for the constructor Car(string VIN). " +
                    $"You've provided the value: {myExceptionalVIN}",
                    ex
                );
            }
